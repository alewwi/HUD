...
import { mapKey } from './utils.js?v=22.99.70';
import { свернутьКоды, НАЗВАНИЯ_КОДОВ } from './codes.js?v=22.99.70';
import { разобратьHUDСырой } from './hud-parser.js?v=22.99.70';
...
// Из снимка их убираем, как дневник: модель их копировала, а что было в
// прошлом ходе, знает из текста ответа и «События» сводки.
...
export function близостьВСнимке(снимок) {
  ...
  if (персонажи.some(c => объект(c) && ИДЁТ_СЦЕНА.some(k => непусто(c[k])))) return true;
  return объект(снимок.us) && непусто(снимок.us.UW);
}
...
const СЛАБЫЕ = [
  /(?<![\p{L}])(?:голая|голый|голые|голой|обнаж[её]нн|нагишом|naked|nude)/iu,
  ...
  /(?<![\p{L}])(?:сос[оу]к|соск|лобок|промежност|nipple)/iu,
  /(?<![\p{L}])(?:смазк|влажн\p{L}* (?:между|внутри)|wet between)/iu,
  /(?<![\p{L}])(?:толч(?:ок|ки|ками|ков)|вход(?:ит|ил) в не[её]|проник|thrust)/iu,
...
export function словаБлизости(тексты) {
  const текст = (Array.isArray(тексты) ? тексты : [тексты]).filter(Boolean).join('\n').replace(hudBlockRe('gi'), ' ');
