...
import { escapeHtml, defeatWI, hudHashSeed, guardTouchSwipe, sanitizeText } from '../utils.js?v=23.13.2';
import { settings } from '../settings.js?v=23.13.2';
import { HUD_AVATAR_COLORS, overrideAvatarUrl } from '../avatars.js?v=23.13.2';
import { G_ICONS } from './icons.js?v=23.13.2';
import { buildMessengerHTML } from './messenger.js?v=23.13.2';
import { avaFace, msgTimeOf, collectCounterparts, parseMsgParties } from './phone-common.js?v=23.13.2';
import { сортироватьЧаты } from './msg-feed.js?v=23.13.2';
import { buildWeatherApp, buildCallsApp, buildMapsApp, buildHealthApp } from './phone-extra.js?v=23.13.2';


import { namesLikelySame, transliterateCyrillic } from '../names.js?v=23.13.2';
...
// доп: { scene, world } — для «Погоды»; журнал звонков лежит в phone.callLog.
export function buildPhoneTabsHTML(chatsMap, uid, isChecked, mainCharName, phoneData, sceneDate, sceneTime, characters, доп = {}) {
  ...
  // Где сейчас владелец: для «Погоды» и отметки «Вы здесь» на карте.
  const владелецКарточка = (Array.isArray(characters) ? characters : []).find(c => c && c['Имя'] && namesLikelySame(c['Имя'], phoneOwner));
