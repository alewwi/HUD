...
// и раскладываем секции вокруг них, в пределах 14–18° по кругу тонов, с разной
...
function собратьТона(тема, вес) {
  ...
  // Случайный цвет с малым весом (например, цвет часов) втаскивал в палитру
  // чужую семью: на розовой теме появлялся оранжевый. Оставляем только те
  // тона, что тема заявляет весомо, и не больше трёх семей.
  const весомые = тона.filter(т => т.в >= 1.2);
  return (весомые.length ? весомые : тона).sort((a, b) => b.в - a.в).slice(0, 3);
}
...
export function собратьПалитру(тема) {
  ...
  // Одна семья — акцент темы: «розовая тема — только розовый и его оттенки».
  // Второй цвет темы (свечение, акцент памяти) в палитру не подмешиваем: на
  // «Вампире» от него появлялся оранжевый, на «Каваи» — бирюза.
  тона = тона.slice(0, 1);
  ...
  // Жёлто-оранжевая полоса бросается в глаза сильнее прочих: на винной теме
  // даже край разброса читался как чужой кремовый или рыжий. Пускаем туда
  // секции, только если тема сама оттуда.
  const ЖЁЛТЫЙ = [12, 70];
  const вПолосе = (h) => { const x = ((h % 360) + 360) % 360; return x >= ЖЁЛТЫЙ[0] && x <= ЖЁЛТЫЙ[1]; };
  const жёлтаяТема = тона.some(т => вПолосе(т.h));

  // Кандидаты: сдвиг от каждого тона темы × светлота × насыщенность.
  const кандидаты = [];
  for (const т of тона) {
    for (let d = -размах; d <= размах; d += 4) {
      if (!жёлтаяТема && вПолосе(т.h + d)) continue;
      for (const dl of (светлаяТема ? [-10, -5, 0, 5, 10] : [-20, -14, -8, -2, 4, 9])) {
        for (const ks of [1.06, .9, .76, .62]) {
          const h = т.h + d, s = Math.max(серая ? 8 : 26, Math.min(88, Sтемы * ks)), l = Lбаза + dl;
          // Светлый красно-оранжевый выглядит кремовым и чужим даже на винной
          // теме: на этих тонах оставляем только ступени потемнее.
          const тёплый = ((h % 360) + 360) % 360;
          if (!жёлтаяТема && тёплый > 0 && тёплый < 30 && dl > -2) continue;
